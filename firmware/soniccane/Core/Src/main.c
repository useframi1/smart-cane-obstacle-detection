...
static channel_t ch[N_CH] = {
    [IDX_FWD]   = {
        ...
        .htim = &htim2,  .tim_channel = TIM_CHANNEL_1,
    },
    [IDX_LEFT]  = {
        .dev = { .I2cHandle = &hi2c1 },
        .xshut_port = XSHUT_L_GPIO_Port, .xshut_pin = XSHUT_L_Pin,
        .htim = &htim1,  .tim_channel = TIM_CHANNEL_1,
    },
    [IDX_RIGHT] = {
        .dev = { .I2cHandle = &hi2c1 },
        .xshut_port = XSHUT_R_GPIO_Port, .xshut_pin = XSHUT_R_Pin,
        .htim = &htim16, .tim_channel = TIM_CHANNEL_1,
    },
};
...
static int sensor_bringup(int i)
{
    ...
    while (!booted) {
        ...
        if (rc != 0) {
            ...
            uart_say(m, n); return 1;
        }
        if (HAL_GetTick() - boot_start > 100) {
            n = snprintf(m, sizeof(m), "S%d boot timeout\r\n", i);
            uart_say(m, n); return 2;
        }
        HAL_Delay(2);
    }

    rc = VL53L1X_SetI2CAddress(ch[i].dev, SENSOR_ADDRS[i]);
    if (rc) { n = snprintf(m, sizeof(m), "S%d setaddr rc=%d\r\n",   i, rc); uart_say(m, n); return 3; }
    ch[i].dev.I2cDevAddr = SENSOR_ADDRS[i];

    rc = VL53L1X_SensorInit(ch[i].dev);
    if (rc) { n = snprintf(m, sizeof(m), "S%d sensorinit rc=%d\r\n", i, rc); uart_say(m, n); return 4; }

    rc = VL53L1X_SetDistanceMode(ch[i].dev, 1);
    if (rc) { n = snprintf(m, sizeof(m), "S%d distmode rc=%d\r\n",   i, rc); uart_say(m, n); return 5; }

    rc = VL53L1X_SetTimingBudgetInMs(ch[i].dev, 50);
    if (rc) { n = snprintf(m, sizeof(m), "S%d tb rc=%d\r\n",         i, rc); uart_say(m, n); return 6; }

    rc = VL53L1X_SetInterMeasurementInMs(ch[i].dev, 50);
    if (rc) { n = snprintf(m, sizeof(m), "S%d imp rc=%d\r\n",        i, rc); uart_say(m, n); return 7; }

    rc = VL53L1X_StartRanging(ch[i].dev);
    if (rc) { n = snprintf(m, sizeof(m), "S%d start rc=%d\r\n",      i, rc); uart_say(m, n); return 8; }
    ...
    if (!ready) { n = snprintf(m, sizeof(m), "S%d no data after init\r\n", i); uart_say(m, n); return 9; }
...
static uint16_t read_one(int i)
{
    ...
    if (rc != 0) { err_count[i]++; i2c_bus_recover(); return ch[i].last_dist; }
    if (!ready) return ch[i].last_dist;
...
static void publish_telemetry(const DistanceMsg_t *msg)
{
    ...
    for (int i = 0; i < N_CH; i++) snap.err_count[i] = err_count[i];
...
static AlertZone_t dist_to_zone(uint16_t dist)
{
    if (dist == 0 || dist > 2000) return ALERT_OFF;
    if (dist < 200)               return ALERT_IMMEDIATE;
    if (dist <= 500)              return ALERT_CLOSE;
    if (dist <= 1000)             return ALERT_MEDIUM;
    return ALERT_FAR;
}
/* USER CODE END 0 */
...
void StartSensorTask(void *argument)
{
  ...
  for(;;)
  {
    ...
    msg.d_fwd   = read_one(IDX_FWD);
    msg.d_left  = read_one(IDX_LEFT);
    msg.d_right = read_one(IDX_RIGHT);
